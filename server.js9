...
app.get('/api/leads/all/:broker_id', (req, res) => {
  db.all(`SELECT * FROM Leads WHERE broker_id = ? ORDER BY created_at DESC`, [req.params.broker_id], (err, rows) => res.json(rows || []));
});
...
app.post('/api/leads/add-manual', (req, res) => {
  ...
  const status = caller_id ? 'Assigned' : 'Unassigned';
  db.run(
    `INSERT INTO Leads (id, broker_id, caller_id, client_name, client_phone, source, budget, location, intent, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [leadId, broker_id, caller_id || null, client_name, client_phone, source, budget, location, intent, status],
    (err) => {
      if(err) return res.status(500).json({ error: err.message });
    ...
    <div id="app" class="min-h-screen pb-12">
      <nav class="bg-indigo-900 text-white p-4 shadow-xl flex flex-col sm:flex-row justify-between items-center gap-3 sticky top-0 z-40">
        <h1 class="text-2xl font-black flex items-center gap-2">
          <i class="fa-solid fa-building-user text-amber-400"></i> PropFlow Data CRM
        </h1>
      ...
      <div class="p-4 max-w-5xl mx-auto mt-4">
        ...
        <!-- FORM 1: BROKER PROFILE -->
        <div v-if="currentTab === 'broker'" class="bg-white p-6 rounded-2xl shadow-md border space-y-4">
          <h2 class="text-xl font-black text-gray-900 border-b pb-2"><i class="fa-solid fa-id-card text-indigo-600"></i> Broker / Agency Profile Form</h2>
          <form @submit.prevent="saveBroker" class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Contact Phone Number</label>
        ...
        <div v-if="currentTab === 'callers'" class="space-y-6">
          ...
          <div class="bg-white p-6 rounded-2xl shadow-md border">
            <h3 class="text-lg font-black text-gray-900 mb-3">Team Members List ({{ telecallers.length }})</h3>
            <div class="divide-y">
        ...
        <!-- FORM 3: ADD LEAD MANUALLY -->
        <div v-if="currentTab === 'leads'" class="space-y-6">
          <div class="bg-white p-6 rounded-2xl shadow-md border space-y-4">
            <h2 class="text-xl font-black text-gray-900 border-b pb-2"><i class="fa-solid fa-address-book text-amber-500"></i> Add Client / Lead Entry Form</h2>
            <form @submit.prevent="addLead" class="space-y-4">
              ...
              <div>
                <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Assign Direct To Telecaller (Optional)</label>
                <select v-model="newLead.caller_id" class="w-full p-3 border rounded-xl font-semibold bg-white">
                  <option value="">-- Keep Unassigned / Open Pool --</option>
              ...
              <table class="w-full text-left text-sm">
                ...
                <tbody>
                  <tr v-for="l in leads" :key="l.id" class="border-b">
                    ...
                    <td class="p-2"><span class="bg-gray-100 text-gray-800 px-2 py-0.5 rounded text-xs">{{ l.source }}</span></td>
                    <td class="p-2">
