...
// ==========================================
// MODULE 2 & 3: UI ENGINE (Single Page Application)
// ==========================================
app.get('/', (req, res) => {
  res.send(`
  <!DOCTYPE html>
  ...
  <body class="bg-gray-100 font-sans">
    <div id="app" class="min-h-screen">
      ...
      <!-- MODULE 2: BROKER ADMIN DASHBOARD -->
      ...
      <!-- MODULE 3: TELECALLER DASHBOARD (MOBILE-OPTIMIZED) -->
      <div v-if="currentView === 'caller'" class="p-4 max-w-md mx-auto space-y-4">
        <!-- Telecaller Selector Switcher for Demo -->
        ...
        <!-- Priority 2 Marketplace: Unassigned Pool -->
        ...
        <div v-for="lead in assignedLeads" :key="lead.id" class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 space-y-3">
          ...
          <!-- Cloud Recording Player (If Available) -->
        ...
        <div v-if="activeCallLead" class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-4 z-50">
          <div class="bg-white w-full max-w-sm rounded-t-2xl sm:rounded-2xl p-6 space-y-4 shadow-2xl animate-bounce-in">
            ...
            <div class="space-y-2.5 pt-2">
              <button @click="submitDisposition('Connected')" class="w-full bg-emerald-50 hover:bg-emerald-100 text-emerald-800 font-bold py-3 rounded-xl border border-emerald-300 flex items-center justify-center gap-2">
                🟢 Connected
              </button>
              <button @click="submitDisposition('Failed')" class="w-full bg-rose-50 hover:bg-rose-100 text-rose-800 font-bold py-3 rounded-xl border border-rose-300 flex items-center justify-center gap-2">
                🔴 Failed / Wrong Number
              </button>
              <button @click="submitDisposition('Top-Notch')" class="w-full bg-amber-50 hover:bg-amber-100 text-amber-800 font-bold py-3 rounded-xl border border-amber-300 flex items-center justify-center gap-2">
                ⭐ Top-Notch (HOT Lead)
              </button>
            </div>
          </div>
